...
async def make_digital_plot(response, time_frame):
    date = []
    open = []
    close = []
    high = []
    low = []
    for key, value in response[f"Time Series (Digital Currency {time_frame})"].items():
        date.append(key)
        open.append(value["1a. open (USD)"])
        high.append(value["2a. high (USD)"])
        low.append(value["3a. low (USD)"])
        close.append(value["4a. close (USD)"])
    prices = pd.DataFrame({
        "date": date,
        "open": open,
        "close": close,
    ...
    ax.grid(which='minor',
            color='k',
            linestyle=':')
    ax.set_title(response["Meta Data"]['3. Digital Currency Name'] + " " + time_frame)
    fig.autofmt_xdate()
    fig.tight_layout()
    path = os.path.join(os.path.abspath('plots'), 'temp.png')
    plt.savefig(os.path.join(os.path.abspath('plots'), 'temp.png'))
